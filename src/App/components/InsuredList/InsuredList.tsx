...
export default function InsuredList({ selectedContractorsIds, setSelectedContractorsIds, approvalId }: InsuredListProps) {
	...
	/** Установка обновления списка */
	const setSearchHandler = (callback: () => void) => {
		setReloadHandler(() => callback);
	};
	// Перезагрузка списка при изменении ID
	useEffect(() => {if(reloadHandler) reloadHandler()}, [approvalId])
		...
		<div className="insured-list">
			<div className='insured-list__list'>
				<CustomList<undefined, InsuredListData>
					...
					getDataHandler={Scripts.getInsuredListRevokeFactory(approvalId)}
					isSelectable={true}
